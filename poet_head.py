...
def get_hidden_poetry(heading,total_length = 1,temperature = 1.8):
    ...
    print()
    print('----- 以「{0}」寫藏頭新詩'.format(heading))
    
    print('----- temperature:', temperature)
    
    result = [] #設一個result串列去接收text_generated，最後要return
    text_generated = []
    hidden_head = list(heading)
    
    
    
    while len(hidden_head) != 0:
        heading_idx=0
        start_string = hidden_head[heading_idx]
        text_generated.append('[CLS]')
        text_generated.append(hidden_head[heading_idx])
        result.append(hidden_head[0]) #將第一個字新增進串列
